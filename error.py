...
with h5py.File("monolithic.h5", mode="r") as f_1, h5py.File(
...
) as f_2, h5py.File("analytic_discretized.h5", mode="r") as f_3:
    for i in range(0, 101):
        ...
        heat_analytic = heat_analytic.reshape(11, 11)
        ...
        errors_half_1_analytic[i] = np.linalg.norm(
            heat_analytic[:, :6].reshape(66) - heat_nested_1
        )
        errors_half_2_analytic[i] = np.linalg.norm(
            heat_analytic[:, 5:].reshape(66) - heat_nested_2
        )
