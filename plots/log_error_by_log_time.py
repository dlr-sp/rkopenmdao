...
class Component(om.ExplicitComponent):
    """
    ...
    def setup(self):
        ...
        self.add_input("lambda", val=-1e2, shape=1, tags=["time_independent_input_var", "lambda"])
    ...
    def compute(self, inputs, outputs):
        ...
        outputs["u_stage"] = ((inputs["lambda"] 
        * (inputs["u"] + delta_t * inputs["acc_stages"] - self.phi(stage_time)) + self.dphi(stage_time)) 
        / (1 - inputs["lambda"]*delta_t*self.options["integration_control"].butcher_diagonal_element))


def solution(time,_lambda):
    return np.sin(time + np.pi/4) + np.exp(_lambda*time)


def component_integration(_class, solution, delta_t, butcher_tableau, quantities):
...
error_data={}
butcher_tableaux = [implicit_euler, two_stage_dirk, four_stage_dirk, five_stage_esdirk, six_stage_esdirk]
for scheme in butcher_tableaux:
    error_data[f'{scheme.name}'] = []
    for step_size in delta_t:
            error_data[f'{scheme.name}'].append(component_integration(Component, solution, step_size, scheme ,["u"]))
...
if rank == 0:
    ...
    for scheme in butcher_tableaux:
        ...
        plt.plot(delta_t, (error_data[f'{scheme.name}'][-1]/delta_t[-1]**p)*(delta_t)**p, "k--", lw=1)
