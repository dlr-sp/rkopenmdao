...
if __name__ == "__main__":
    ...
    for butcher_tableau in BUTCHER_TABLEAUX:
        ...
        if parsed_args.adaptive:
            ...
            file_name2 = f"non_adaptive_test_{butcher_tableau.name}"
            file_name2 = file_name2.replace(" ", "_")
            file_name2 = file_name2.replace(",", "")
            file_name2 = file_name2.lower()
            file_path = FOLDER_PATH_ADAPT / f"{file_name2}.h5"

            time_non_adaptive = {}
            solutions_non_adaptive = {}
            ...
            with h5py.File(
                ...
                for i, quantity in enumerate(QUANTITY):
                    solutions_non_adaptive[quantity] = {}
                    non_adaptive_error_data[quantity] = {}
                    group = f[quantity]
                    for key in group.keys():
                        solutions_non_adaptive[quantity][int(key)] = group[key][0]
                        non_adaptive_error_data[quantity][int(key)] = np.abs(
                            kaps_solution(time_non_adaptive[int(key)])[i]
                            - solutions_non_adaptive[quantity][int(key)]
                        )
            time_non_adaptive = dict(sorted(time_non_adaptive.items()))
            for quantity in QUANTITY:
                solutions_non_adaptive[quantity] = dict(
                    sorted(solutions_non_adaptive[quantity].items())
                )
    ...
    if rank == 0:
        ...
        for i, scheme in enumerate(BUTCHER_TABLEAUX):
            ...
            if (i + 1) % 2 != 0:
                print(scheme.name)
                err = local_error_data[f"{scheme.name}"]["0.02"]
