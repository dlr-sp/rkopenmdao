...
@pytest.mark.parametrize(
    "butcher_tableau",
    [implicit_euler, embedded_second_order_two_stage_sdirk, runge_kutta_four],
)
...
def test_component_integration(
    ...
    assert result == pytest.approx(
        test_functor(initial_time + 0.01, initial_values, initial_time), rel=1e-4
    )
...
def test_component_integration_adaptive(
    test_class,
    test_functor,
    initial_time,
    initial_values,
...
def test_time_integration_partials(
    ...
    # integration_control = StepTerminationIntegrationControl(0.001, 10, initial_time)
    integration_control = StepTerminationIntegrationControl(0.001, 3, initial_time)
    time_integration_prob = om.Problem()
    time_integration_prob.model.add_subsystem(
        "test_comp", test_class(integration_control=integration_control)
    )
...
@pytest.mark.parametrize("test_controller", error_controller_list)
def test_time_integration_partials_adaptive(
    test_class,
    initial_time,
    butcher_tableau,
    checkpointing_implementation,
    ...
    if checkpointing_implementation == NoCheckpointer:
        ...
        data = runge_kutta_prob.check_partials()
        _check_partials_wo_fd(data)


def _check_partials_wo_fd(jac_data, tol=1e-6):
    """
    ...
    for i in ["x_initial"]:
        ...
        # Absolute :
        assert np.all(np.abs(fwd - rev) < tol)
        # Relative
        assert np.all(np.abs(fwd - rev) / np.abs(fwd) < tol)
