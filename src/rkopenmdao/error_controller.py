...
class ErrorControllerStatus:
    ...
    step_size_suggestion: float
        Suggestion for the step size of the next calculated time step.
    acceptance: bool
        Flag for the acceptance of the current time step.
    """
...
class ErrorControllerConfig:
    ...
    ----------
    tol: float
        Tolerance for acceptance of steps and control of step sizes.
    lower_bound: float
        Lower cutoff for step sizes.
    upper_bound: float
        Upper cutoff for step sizes.
    safety_factor: float
        Factor on step size to reduce number of rejected steps.
    max_iter: int
        Maximal number of retries on encountering rejected steps.
    """
...
class ErrorController:
    ...
    def __init__(
        ...
        self.b = b
        # -----------
        self._inner_most = True  # Indicator if it is not associated with a decorator
        self._name = "Outermost " + name
        self._width = 0
    ...
    def __call__(
        ...
        """
        Given information of current time step, assess acceptance of current time step.
    ...
    def _run(
        ...
        """
        Estimates next possible step size for a given error measure and returns whether
        the next step size meets the tolerance.
    ...
    def _estimate_next_step_function(
        ...
        """
        Computes estimate for step size of next time step based on current and past
        step size and error information.
        ...
        -------
        delta_t_new: float
            Step size estimate for next time step.
        """
...
class ErrorControllerDecorator(ErrorController):
    ...
    a different error controller as a fallback in case of failure.
