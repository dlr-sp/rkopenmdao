...
class RungeKuttaIntegrator(om.ExplicitComponent):
    ...
    def _check_num_checkpoints_in_revolver_options(self):
        if "n_checkpoints" not in self.options["revolver_options"].keys() and self.options[
            "revolver_type"
        ] not in ["MultiLevel", "Base"]:
    ...
    def _setup_variables(self):
        self._functional_quantities = self.options["functional_coefficients"].list_quantities()
    ...
    def _setup_arrays(self):
        ...
        # were originally all resizes, but this interfered with cProfile for some unknown reason

        self._serialized_state = np.zeros(self._numpy_array_size)
        self._accumulated_stages = np.zeros(self._numpy_array_size)
        self._stage_cache = np.zeros((butcher_tableau.number_of_stages(), self._numpy_array_size))
        self._serialized_state_perturbations = np.zeros(self._numpy_array_size)
        self._serialized_state_perturbations_from_functional = np.zeros(self._numpy_array_size)
        ...
        # self._serialized_state.resize(self._numpy_array_size)
        # self._accumulated_stages.resize(self._numpy_array_size)
        # self._stage_cache.resize(
        #     (butcher_tableau.number_of_stages(), self._numpy_array_size)
        # )
        # self._serialized_state_perturbations.resize(self._numpy_array_size)
        # self._serialized_state_perturbations_from_functional.resize(
        #     self._numpy_array_size
        # )
        # self._accumulated_stage_perturbations.resize(self._numpy_array_size)
        # self._stage_perturbations_cache.resize(
        #     (butcher_tableau.number_of_stages(), self._numpy_array_size)
        # )

        # self._postprocessing_state.resize(self._numpy_postproc_size)
        # self._postprocessing_state_perturbations.resize(self._numpy_postproc_size)

        # self._functional_part.resize(self._numpy_functional_size)
        # self._functional_part_perturbations.resize(self._numpy_functional_size)

    def _setup_checkpointing(self):
        butcher_tableau: ButcherTableau = self.options["butcher_tableau"]
        self._check_num_checkpoints_in_revolver_options()

        self._serialized_old_state_symbol = RungeKuttaIntegratorSymbol(self._numpy_array_size)
        self._serialized_new_state_symbol = RungeKuttaIntegratorSymbol(self._numpy_array_size)
    ...
    def _stage_preparation(self, stage):
        delta_t = self.options["integration_control"].delta_t
        time = self.options["integration_control"].step_time_old
        self.options["integration_control"].stage = stage
        if stage != 0:
            self._accumulated_stages = self._runge_kutta_scheme.compute_accumulated_stages(
    ...
    def _run_step_postprocessing_phase(self):
        if self.options["postprocessing_problem"] is not None:
            self._postprocessing_state = self._postprocessor.postprocess(self._serialized_state)
    ...
    def _compute_jacvec_fwd_postprocessing_phase(self):
        if self.options["postprocessing_problem"] is not None:
            ...
            self._postprocessing_state_perturbations = self._postprocessor.postprocess_jacvec(
    ...
    def _compute_jacvec_fwd_run_steps_time_integration_phase(self):
        ...
        self._serialized_state_perturbations = self._runge_kutta_scheme.compute_step_jacvec(
            delta_t,
            self._serialized_state_perturbations,
            self._stage_perturbations_cache,
        )
    ...
    def _compute_jacvec_fwd_translate_to_om_vector_phase(
        ...
        if self._functional_quantities:
            self._add_functional_part_to_om_vec(self._functional_part_perturbations, d_outputs)

    def _compute_jacvec_product_rev(self, inputs, d_inputs, d_outputs):
        self._disable_write_out = True
        self._to_numpy_array(inputs, self._serialized_state)
        if not np.array_equal(self._cached_input, self._serialized_state) or self._revolver is None:
            outputs = self._vector_class("nonlinear", "output", self)
            self.compute(inputs, outputs)

        self._to_numpy_array(d_outputs, self._serialized_state_perturbations)
        self._to_numpy_array_postprocessing(d_outputs, self._postprocessing_state_perturbations)
        if self.options["postprocessing_problem"] is not None:
            self._serialized_state_perturbations += (
        ...
        self._reverse_operator.serialized_state_perturbations = self._serialized_state_perturbations

        self._revolver.apply_reverse()

        self._from_numpy_array(self._reverse_operator.serialized_state_perturbations, d_inputs)

        self._revolver = None
        self._configure_write_out()

    def _run_step_jacvec_rev(self, step, serialized_state, serialized_state_perturbations):
        self._serialized_state = serialized_state
        ...
        for stage in range(butcher_tableau.number_of_stages()):
            self._stage_preparation(stage)
            inputs_cache[stage] = self.options["time_stage_problem"].model._vector_class(
                "nonlinear", "input", self.options["time_stage_problem"].model
            )
            inputs_cache[stage].set_vec(self.options["time_stage_problem"].model._inputs)
            outputs_cache[stage] = self.options["time_stage_problem"].model._vector_class(
                "nonlinear", "output", self.options["time_stage_problem"].model
            )
            outputs_cache[stage].set_vec(self.options["time_stage_problem"].model._outputs)
        # backward iteration
        ...
        for stage in range(butcher_tableau.number_of_stages() - 1, -1, -1):
            ...
            joined_perturbations = (
                self._runge_kutta_scheme.join_new_state_and_accumulated_stages_perturbations(
                    stage,
                    self._serialized_state_perturbations,
                    self._stage_perturbations_cache,
                )
    ...
    def _add_functional_perturbations_to_state_perturbations(
        ...
        functional_coefficients: FunctionalCoefficients = self.options["functional_coefficients"]
        postprocessing_functional_perturbations = np.zeros(self._numpy_postproc_size)
        for quantity in self._functional_quantities:
            start_functional = self._quantity_metadata[quantity]["numpy_functional_start_index"]
            end_functional = self._quantity_metadata[quantity]["numpy_functional_end_index"]
    ...
    def _get_functional_contribution_from_om_output_vec(self, d_outputs: OMVector):
        for quantity in self._functional_quantities:
            ...
            self._functional_part_perturbations[start:end] = d_outputs[quantity + "_functional"]
        ...
        for quantity in self.options["time_integration_quantities"]:
            ...
            if not (
                (
                    ...
                    and (self._quantity_metadata[quantity]["accumulated_stage_var"] is not None)
                    and (self._quantity_metadata[quantity]["stage_output_var"] is not None)
                )
                or (
                    (self._quantity_metadata[quantity]["step_input_var"] is None)
                    and (self._quantity_metadata[quantity]["accumulated_stage_var"] is None)
                    and self._quantity_metadata[quantity]["stage_output_var"] is not None
                )
            ):
                raise SetupError(
            ...
            self._quantity_metadata[quantity]["numpy_start_index"] = self._numpy_array_size
            self._numpy_array_size += np.prod(self._quantity_metadata[quantity]["shape"])
            self._quantity_metadata[quantity]["numpy_end_index"] = self._numpy_array_size
    ...
    def _add_functional_quantity_metadata(self, quantity):
        if quantity in self._functional_quantities:
            ...
            self._numpy_functional_size += np.prod(self._quantity_metadata[quantity]["shape"])
            self._quantity_metadata[quantity][
    ...
    def _extract_postprocessing_quantity_metadata_from_postprocessing_problem(
        ...
        for var, metadata in quantity_outputs.items():
            if var in postproc_output_vars:
                ...
                self._numpy_postproc_size += np.prod(self._quantity_metadata[quantity]["shape"])
                self._quantity_metadata[quantity][
    ...
    def _extract_time_integration_quantity_metadata_from_postprocessing_problem(
        ...
        for var, metadata in quantity_inputs.items():
            if var in postproc_input_vars:
                ...
                assert metadata["global_shape"] == self._quantity_metadata[quantity]["global_shape"]
    ...
    def _extract_quantity_metadata_from_time_stage_problem(
        ...
        for var, metadata in quantity_outputs.items():
            ...
            if (
                ...
                self._quantity_metadata[quantity]["local_indices_start"] = start = np.sum(
    ...
    def _extract_quantity_metadata_or_check_for_consistency(self, quantity: str, metadata: dict):
    ...
    def _add_time_integration_inputs_and_outputs(self, quantity):
        ...
        self.add_input(
            quantity + "_initial",
            shape=self._quantity_metadata[quantity]["shape"],
            val=time_stage_problem.get_val(self._quantity_metadata[quantity]["step_input_var"])
    ...
    def _to_numpy_array_postprocessing(self, om_vector: OMVector, np_postproc_array: np.ndarray):
        name_suffix = "_final"
        for quantity, metadata in self._quantity_metadata.items():
    ...
    def _from_numpy_array_postprocessing(self, np_postproc_array: np.ndarray, om_vector: OMVector):
        name_suffix = "_final"  # postprocessing variables are only in output, not input
        for quantity, metadata in self._quantity_metadata.items():
            if metadata["type"] == "postprocessing":
                start = metadata["numpy_postproc_start_index"]
                end = metadata["numpy_postproc_end_index"]
                om_vector[quantity + name_suffix] = np_postproc_array[start:end].reshape(
                    metadata["shape"]
                )
    ...
    def _get_functional_contribution(
        ...
        for quantity in self._functional_quantities:
            ...
            end_functional = self._quantity_metadata[quantity]["numpy_functional_end_index"]
            if self._quantity_metadata[quantity]["type"] == "time_integration":
                start_time_stepping = self._quantity_metadata[quantity]["numpy_start_index"]
                ...
                end_postprocessing = self._quantity_metadata[quantity]["numpy_postproc_end_index"]
                contribution[start_functional:end_functional] = (
                    functional_coefficients.get_coefficient(timestep, quantity)
    ...
    def _add_functional_part_to_om_vec(
        ...
        for quantity in self._functional_quantities:
            ...
            om_vector[quantity + "_functional"] = functional_numpy_array[start:end].reshape(
                self._quantity_metadata[quantity]["shape"]
            )
